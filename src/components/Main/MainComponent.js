...
function MyRoutes(props) {
  const routes = props.pages.map((each) => {
    ...
    if (each.exact)
      return (
        <Route key={each.slug} exact path={path} component={each.component} />
      );
    else if (each.status)
      return (
        <Route
      ...
      return <Route key={each.slug} path={path} component={each.component} />;
  });
...
export default Size({ component: true, mobile: true, orientation: true })(Main);
